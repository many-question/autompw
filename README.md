...
AutoMPW 是一个用于 MPW 流片拼版的 Python CLI 工具。它可以把多个子设计 GDS 按指定坐标放入一个 MPW 顶层版图中，生成 framework GDS、dummy blocker GDS，基于模板渲染 Calibre dummy fill deck，并最终拼合输出交付流片用的 MPW GDS。
...
- 自动根据当前任务生成 Calibre 运行期字段：
  ...
  - summary report
  - chip window 坐标
- 为每个子设计生成 blank dummy blocker GDS。
- 拼合 framework、dummy fill、原始子设计 GDS 和可选 dummy blocker GDS，输出最终 MPW GDS。
...
autompw dummy-blockers CONFIG.yaml [--dry-run]
...
为每个子设计生成 blank GDS，按每个 block 的尺寸渲染独立 Calibre deck，并在非 `--dry-run` 模式下调用 Calibre。

```bash
autompw dummy-blockers examples/mpw_2512.yaml --dry-run
autompw dummy-blockers examples/mpw_2512.yaml
```

每个子设计 dummy blocker 的运行期信息由程序自动生成：

- 输入 GDS：该子设计的 blank GDS
- 输入 topcell：`DUMMY_<design_name>`
- chip window：该子设计尺寸
...
`calibre` 是全局配置，不只属于 `dummy-fill` 阶段。MPW dummy fill 和子设计 dummy blocker 都复用这套配置。
...
如果希望最终拼版时用 dummy blocker 替换某个子设计，设置：
